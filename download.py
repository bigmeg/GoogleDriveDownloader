...
class SmartDL:
    ...
    def __init__(self, urls, dest=None, progress_bar=True, threads=5, logger=None, connect_default_logger=False):
        global DEFAULT_LOGGER_CREATED

        self.mirrors = [urls] if isinstance(urls, str) else urls
        self.url = self.mirrors.pop(0)

        self.filename = fn = urllib.parse.unquote(os.path.basename(urlparse(self.url).path))
    ...
    def get_summary_dict(self):
        return {"filename": self.filename,
                "status": self.status,
                "completed_size": self.get_dl_size(human=True),
                "file_size": utils.sizeof_human(self.filesize),
                "speed": self.get_speed(human=True),
                "progress": self.get_progress(),
                "eta": self.get_eta(human=True)}
    ...
    def start(self, blocking=None):
        ...
        if len(args) > 1:
            self.logger.info(
                "Launching %d threads (downloads %s/Thread)." % (len(args), utils.sizeof_human(bytes_per_thread)))
        else:
        ...
        self.post_threadpool_thread = threading.Thread(target=post_threadpool_actions, args=(
        self.pool, [[(self.dest + ".%.3d" % i) for i in range(len(args))], self.dest], self.filesize, self))
    ...
    def isSuccessful(self):
        ...
        while self.status != 'finished':
            ...
            if n >= 15:
                raise RuntimeError(
                    "The download task must be finished in order to see if it's successful. (current status is %s)" % self.status)
...
class ControlThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.obj.pool.done():
            ...
            if self.progress_bar:
                if self.obj.filesize:
                    ...
                    utils.sizeof_human(self.shared_var.value), utils.sizeof_human(self.obj.filesize),
                    utils.sizeof_human(self.dl_speed),
                    utils.progress_bar(1.0 * self.shared_var.value / self.obj.filesize),
                    self.shared_var.value * 100.0 / self.obj.filesize, utils.time_human(self.eta, fmt_short=True))
                else:
                    status = r"[*] %s / ??? MB @ %s/s   " % (
                    utils.sizeof_human(self.shared_var.value), utils.sizeof_human(self.dl_speed))
        ...
        if self.progress_bar:
            if self.obj.filesize:
                print(r"[*] %s / %s @ %s/s %s [100%%, 0s left]    " % (
                utils.sizeof_human(self.obj.filesize), utils.sizeof_human(self.obj.filesize),
                utils.sizeof_human(self.dl_speed), utils.progress_bar(1.0)))
            else:
                print(r"[*] %s / %s @ %s/s    " % (
                utils.sizeof_human(self.shared_var.value), utils.sizeof_human(self.shared_var.value),
                utils.sizeof_human(self.dl_speed)))
...
def post_threadpool_actions(pool, args, expected_filesize, SmartDL_obj):
    ...
    if expected_filesize:  # if not zero, etc expected filesize is not known
        ...
        if diff > 4 * threads:
            SmartDL_obj.logger.warning(
                'Diff between downloaded files and expected filesizes is %dKB. Retrying...' % diff)
...
def download(url, dest, startByte=0, endByte=None, headers=None, timeout=4, shared_var=None, thread_shared_cmds=None,
    ...
    try:
        ...
        if e.code == 416:
            ...
            if retries > 0:
                logger.warning(
                    "Thread didn't got the file it was expecting. Retrying (%d times left)..." % (retries - 1))
                time.sleep(5)
                return download(url, dest, startByte, endByte, headers, timeout, shared_var, thread_shared_cmds, logger,
                                retries - 1)
            else:
                raise
        else:
